...
function populate(n) {
    ...
    for ( var j = 0; j < 25; j++ ) {
        for ( var i = 0; i < 10; i++ ) {
            x = (j % 2) == 0 ? -100 : i * 10 -150;
            y = j * 2;
            z = (j % 2) == 0 ? i * 10 - 50 : 0;

            config[3] = group3;
            bodys[index] = new OIMO.Body({type:'box', size:[w,h,d], pos:[x,y,z], rot:[90, (j % 2)== 0 ? 90 : 0, 0], move:true, world:world, config:config});
            var pos = Math.floor(Math.random() * matPocky.length );
            meshs[index] = new THREE.Mesh( buffgeoCylinder, matPocky[pos] );
