...
/*
    // ループによるコピー
    var pos = 0;
    for (var i = 0; i < colorBuffers.length; i += N) {
        if (i < (colorBuffers.length - N)) {
            pos = i + N;
        } else {
            pos = i - (colorBuffers.length - N);
        }
        colorBuffersTemp[i + 0] = colorBuffers[pos + 0];
        colorBuffersTemp[i + 1] = colorBuffers[pos + 1];
        colorBuffersTemp[i + 2] = colorBuffers[pos + 2];
        colorBuffersTemp[i + 3] = colorBuffers[pos + 3];
    }
*/
    // subarray / set によるコピー
    var srcBlock1 = colorBuffers.subarray(N, 1024);
    var srcBlock2 = colorBuffers.subarray(0, N);
    
    var dstBlock1 = colorBuffersTemp.subarray(0, 1024 - N);
    var dstBlock2 = colorBuffersTemp.subarray(1024 - N, 1024);
