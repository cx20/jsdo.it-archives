...
function init() {
    ...
    parser.parse("../../assets/y/l/j/y/yljy3.vox").then(function(voxelData) {
        tmpVoxelData = voxelData;
        X_START_POS = -voxelData.size.x * DOT_SIZE / 2;
        Y_START_POS = -voxelData.size.z * DOT_SIZE / 2;
        Z_START_POS = -voxelData.size.y * DOT_SIZE / 2;
...
function render() {
    var i = 0;
    for ( var z = 0; z < tmpVoxelData.size.z; z++ ) {
        for ( var y = 0; y < tmpVoxelData.size.y; y++ ) {
            for ( var x = 0; x < tmpVoxelData.size.x; x++ ) {
                var voxel = tmpArray[x + "," + y + "," + z];
                if ( voxel !== undefined ) {
                    meshArray[i].position.x = ((voxel.x+cnt) % tmpVoxelData.size.x) * DOT_SIZE + X_START_POS;
                    meshArray[i].position.y = voxel.z * DOT_SIZE + Y_START_POS;
                    meshArray[i].position.z = voxel.y * DOT_SIZE + Z_START_POS;
                    i++;
                }
