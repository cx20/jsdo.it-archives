空耳メーカーは「空耳辞書」を用いて「日本語」→「英語（空耳）」への変換を行う為のコンバータです。
一応、漢字も対応していますが、漢数字が弱いので、うまくいかない場合はひらがなにて入力お願いします。

＜機能＞
・漢字→ひらがな変換（Yahoo! テキスト解析:ルビ振りAPI使用）
・読み上げ機能（Speak.js を使用。Firefox, Chrome のみ対応）

＜変更履歴＞
2013/04/29 v0.02 漢字→ひらがな変換対応。
2013/04/27 v0.01 初版作成。読み上げ機能対応。

 ＜参考＞
■ 空耳メーカー（Soramimi Maker）for Web - ver0.09
http://cx20.main.jp/edu/javascript/SoramimiMaker/
■ 空耳辞書 開発者ブログ
http://javascript.g.hatena.ne.jp/cx20/20090224/1235487702

＜使用ライブラリ＞
■ テキスト解析:ルビ振りAPI - Yahoo!デベロッパーネットワーク
http://developer.yahoo.co.jp/webapi/jlp/furigana/v1/furigana.html
■ XMLのWebAPIを爆速で使いこなせるフレームワーク - Yahoo! JAPAN Tech Blog
http://techblog.yahoo.co.jp/programming/bakusoku-yql/

■ mattytemple/speak-js · GitHub
https://github.com/mattytemple/speak-js
