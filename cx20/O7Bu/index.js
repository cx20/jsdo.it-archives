...
function createMesh() {
    var cdrom_sides_geo =
        new THREE.CylinderGeometry(1.0, 1.0, 1.0, 32.0, 10.0, true);
    var cdrom_cap_geo = new THREE.Geometry();
    var r = 1.0;
    for (var i = 0; i < 32; i++) {
        var a = i * 1 / 32 * Math.PI * 2;
        var z = Math.sin(a);
        var x = Math.cos(a);
        var a1 = (i + 1) * 1 / 32 * Math.PI * 2;
        var z1 = Math.sin(a1);
        var x1 = Math.cos(a1);
        cdrom_cap_geo.vertices.push(
            new THREE.Vector3(0, 0, 0),
            new THREE.Vector3(x * r, 0, z * r),
            new THREE.Vector3(x1 * r, 0, z1 * r)
        );
        cdrom_cap_geo.faceVertexUvs[0].push([
            new THREE.Vector2(0.5, 0.5),
            new THREE.Vector2(x / 2 + 0.5, z / 2 + 0.5),
            new THREE.Vector2(x1 / 2 + 0.5, z1 / 2 + 0.5)
        ]);
        cdrom_cap_geo.faces.push(new THREE.Face3(i * 3, i * 3 + 1, i * 3 + 2));
    }

    cdrom_cap_geo.computeFaceNormals();
    cdrom_cap_geo.computeVertexNormals();

    var cdrom_sides_texture      = THREE.ImageUtils.loadTexture("../../assets/2/1/X/X/21XXW.jpg"); // side.jpg
    var cdrom_cap_texture_top    = THREE.ImageUtils.loadTexture("../../assets/k/h/9/J/kh9Jx.png"); // windows95-front.png
    var cdrom_cap_texture_bottom = THREE.ImageUtils.loadTexture("../../assets/o/4/L/l/o4LlU.png"); // windows95-back.png

    var cdrom_sides_mat = new THREE.MeshLambertMaterial({
        map: cdrom_sides_texture
    });
    var cdrom_sides = new THREE.Mesh(cdrom_sides_geo, cdrom_sides_mat);

    var cdrom_cap_mat_top = new THREE.MeshLambertMaterial({
        map: cdrom_cap_texture_top
    });
    var cdrom_cap_mat_bottom = new THREE.MeshLambertMaterial({
        map: cdrom_cap_texture_bottom
    });
    var cdrom_cap_top = new THREE.Mesh(cdrom_cap_geo, cdrom_cap_mat_top);
    var cdrom_cap_bottom = new THREE.Mesh(cdrom_cap_geo, cdrom_cap_mat_bottom);
    cdrom_cap_top.position.y = 0.5;
    cdrom_cap_top.rotation.x = Math.PI;
    cdrom_cap_bottom.position.y = -0.5;
    cdrom_cap_bottom.rotation.y = Math.PI;

    var cdrom = new THREE.Object3D();
    cdrom.add(cdrom_sides);
    cdrom.add(cdrom_cap_top);
    cdrom.add(cdrom_cap_bottom);

    cdrom.rotation.x = Math.PI * 0.5;
