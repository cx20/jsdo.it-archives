...
function init() {
    ...
    //var image = THREE.ImageUtils.loadTexture('carrot.jpg');
    var image = THREE.ImageUtils.loadTexture('../../assets/l/d/3/u/ld3uq.jpg');
    matBox = new THREE.MeshLambertMaterial( {  map: basicTexture(), name:'box' } );
    matCarrot = new THREE.MeshLambertMaterial( {  map: image, name:'carrot' } );
...
function populate(n) {
    ...
    while (i--){
        ...
        meshs[i] = new THREE.Mesh( buffgeoCylinder, matCarrot );
...
function updateOimoPhysics() {
    ...
    while (i--){
        ...
        if(!body.sleeping){
            ...
            if(mesh.material.name === 'carrot') mesh.material = matCarrot; 
