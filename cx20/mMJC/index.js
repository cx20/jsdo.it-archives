...
/*
    paddel = new THREE.Mesh( buffgeoBox, matGroundTrans );
    paddel.scale.set( 20, 40, 60 );
    scene.add( paddel );
*/
...
function populate(n) {
    ...
    var max = dataSet.length; // document.getElementById("MaxNumber").value;
...
*/
    for ( var i = 0; i < 20; i++ ) {
        var ground = new OIMO.Body({size:[10, 40, 400], pos:[i * 10,i * 8,0], world:world, config:config});
        addStaticBox([10, 40, 400], [i * 10,i * 8,0], [0,0,0]);
    }
...
/*    
    ...
    while (i--){
        ...
        if(t===1){
            ...
            var material = new THREE.MeshLambertMaterial( { color: Math.floor(Math.random() * 0xffffff) } );
            meshs[i] = new THREE.Mesh( buffgeoBox, material );
...
*/
    for ( var i = 0; i < dataSet.length; i++ ) {
        x = 150;
        y = 100 + (15 - Math.floor(i / 16)) * 64;
        z = -170 + (i % 16) * 22 + Math.random()*10;
        w = 20; // + Math.random()*10;
        h = 20; // + Math.random()*10;
        d = 20; // + Math.random()*10;
        config[4] = all;
