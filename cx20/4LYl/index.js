...
function init() {
    ...
    parser.parse("../../assets/y/l/j/y/yljy3.vox").then(function(voxelData) {
        ...
        for ( var i = 0; i  < voxelData.voxels.length; i++ ) {
            ...
            // velocities
            velocities[ i * 3 + 0 ] = Math.random();
            velocities[ i * 3 + 1 ] = Math.random();
            velocities[ i * 3 + 2 ] = Math.random();
...
function render() {
    for ( var i = 0; i < geometry.attributes.velocity.array.length; i += 3 ) {
        var vx = geometry.attributes.velocity.array[i + 0];
        var vy = geometry.attributes.velocity.array[i + 1];
        var vz = geometry.attributes.velocity.array[i + 2];

        var px = geometry.attributes.position.array[i + 0];
        var py = geometry.attributes.position.array[i + 1];
        var pz = geometry.attributes.position.array[i + 2];
        
        px += vx;
        py += vy;
        pz += vz;

        if      (px <-PARTICLE_RANGE) { px =-PARTICLE_RANGE; vx *= -1; }
        else if (px > PARTICLE_RANGE) { px = PARTICLE_RANGE; vx *= -1; }
        if      (py <-PARTICLE_RANGE) { py =-PARTICLE_RANGE; vy *= -1; }
        else if (py > PARTICLE_RANGE) { py = PARTICLE_RANGE; vy *= -1; }
        if      (pz <-PARTICLE_RANGE) { pz =-PARTICLE_RANGE; vz *= -1; }
        else if (pz > PARTICLE_RANGE) { pz = PARTICLE_RANGE; vz *= -1; }

        geometry.attributes.position.array[i + 0] = px;
        geometry.attributes.position.array[i + 1] = py;
        geometry.attributes.position.array[i + 2] = pz;

        geometry.attributes.velocity.array[i + 0] = vx;
        geometry.attributes.velocity.array[i + 1] = vy;
        geometry.attributes.velocity.array[i + 2] = vz;
    }
