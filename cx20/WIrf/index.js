// forked from cx20's "Three.js + Oimo.js でひな壇に紅白餅を落としてみるテスト" http://jsdo.it/cx20/DollsFestival2015
...
function init() {
    ...
    var texture = loader.load("../../assets/0/n/u/N/0nuNS.jpg"); // grass.jpg
    ...
    for (var deg = 0; deg <= 180; deg += 6) {
        ...
        var point = new THREE.Vector2((0.72 * scale + 0.08 * scale * Math.cos(rad)) * Math.sin(rad), -scale * Math.cos(rad)); // the "egg equation"
        points.push(point);

    }
    buffgeoSphere = new THREE.LatheBufferGeometry( points, 32 );

    buffgeoBox = new THREE.BufferGeometry();
    buffgeoBox.fromGeometry(new THREE.BoxGeometry(1, 1, 1));

    ballTexture = loader.load("../../assets/A/M/8/G/AM8GU.jpg"); // egg.jpg
    matSphere = new THREE.MeshPhongMaterial({
        map: ballTexture,
        color: 0xffffff,
        specular: 0x111111,
        shininess: 20,
