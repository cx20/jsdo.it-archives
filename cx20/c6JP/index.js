// forked from cx20's "forked: Suikafall  (WebGL2 + Oimo.js)" http://jsdo.it/cx20/p1cf
...
for (var latNumber = 0; latNumber <= latitudeBands; latNumber++) {
    ...
    for (var longNumber = 0; longNumber <= longitudeBands; longNumber++) {
        ...
        var x = h * cosPhi * sinTheta;
        var y = h * cosTheta;
        var z = h * sinPhi * sinTheta;
        position = position.concat([x, y, z]);
        normal = normal.concat([x, y, z]);
...
//img.src = "suika.png";
img.src = "../../assets/A/a/O/P/AaOPs.png";  // 256x256
...
//var ground = new OIMO.Body({size:[3.5, 0.1, 3.5], pos:[0, -3, 0], world:world});
var ground = new OIMO.Body({size:[6.5*3, 0.1, 6.5*3], pos:[0, -3*3, 0], world:world});
var bodys = [];
for (var i = 0; i < max; i++) {
    var p = genPosition();
    //bodys[i] = new OIMO.Body({type:'box', size:[h, h, h], pos:[p.x, p.y, p.z], move:true, world:world});
    bodys[i] = new OIMO.Body({type:'sphere', size:[h], pos:[p.x, p.y, p.z], move:true, world:world});
