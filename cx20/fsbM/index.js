// forked from cx20's "forked: LiquidFun を試してみるテスト（その２）" http://jsdo.it/cx20/uFEe
...
*/
    for (var i = 0; i < dataSet1.length; i++) {
        if (dataSet1[i] != "無") {
            ...
            var circle = new b2CircleShape();
            var pgd = new b2ParticleGroupDef();
            circle.position.Set(-3.0 + x * 0.25, +3.0 + y * 0.25);
            circle.radius = 0.10;
            pgd.shape = circle;
            var color = getRgbColor(dataSet1[i]);
    ...
    for (var i = 0; i < dataSet2.length; i++) {
        if (dataSet2[i] != "無") {
            var x = i % 32;
            var y = (31 - Math.floor(i / 32));
            var circle = new b2CircleShape();
            var pgd = new b2ParticleGroupDef();
            circle.position.Set(-3.0 + x * 0.25, +4.0 + y * 0.25);
            circle.radius = 0.10;
            pgd.shape = circle;
            var color = getRgbColor(dataSet2[i]);
            pgd.color.Set(color.r, color.g, color.b, color.a);
            particleSystem.CreateParticleGroup(pgd);
        }
    }
