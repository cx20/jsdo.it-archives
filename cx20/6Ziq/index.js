// forked from cx20's "Velocity.js でドット絵を動かしてみるテスト" http://jsdo.it/cx20/4GvN
// forked from snorpey's "Block Animation Demo" http://codepen.io/snorpey/pen/wpKLl


var DOT_SIZE = 14;
var X_DOT_SIZE = DOT_SIZE;   // １マスのサイズ：14x7
var Y_DOT_SIZE = DOT_SIZE/2; // １マスのサイズ：14x7
var X_START_POS = 574;       // 開始位置がマス目に揃うよう調整
var Y_START_POS = 198;       // 開始位置がマス目に揃うよう調整
var Z_START_POS = 0;
...
function dataSetToBlock( dataSet ) {
	...
	for ( var i = 0; i < dataSet.length; i++ ) {
		var x = (i % 16) * X_DOT_SIZE + X_START_POS;
		var y = Math.floor( i / 16 ) * Y_DOT_SIZE + Y_START_POS;
		var z = 0;
		...
		if ( dataSet[i] != "無" ) {
			result.push( {
                ...
                "color": color, 
                "position": {
                    "x": x - Math.floor(i / 16) * X_DOT_SIZE, 
                    "y": y + (i % 16) * Y_DOT_SIZE, 
                    "z": z
                },
                "type": "solid",
