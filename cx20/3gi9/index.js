...
function drawKeyboard() {
	...
	for (var i in keyboardHash) {
		...
		if ( keyboard.color == "white" ) {
			s.rect(keyboard.x, keyboard.y - 80, 24, 100).attr({fill: 'white', stroke:'black', 'fill-opacity': 0.0});
		} else if ( keyboard.color == "black" ) {
			s.rect(keyboard.x, keyboard.y - 50, 15, 70).attr({fill: 'black'});
		}
	}
}
...
function drawMidiKey(midi) {
	...
	if ( keyboard.color == "white" ) {
		s.rect(keyboard.x, keyboard.y - 80, 24, 100).attr({fill: '#0000ff', 'fill-opacity': 0.5});
	} else if ( keyboard.color == "black" ) {
		s.rect(keyboard.x, keyboard.y - 50, 15, 70).attr({fill: '#ff0000'});
	}
}
