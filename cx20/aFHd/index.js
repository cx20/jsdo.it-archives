...
*/ 
        ...
        x = Math.cos(2 * Math.PI * (pos / N) + floor * Math.PI) * 40 - 100;
        z = Math.sin(2 * Math.PI * (pos / N) + floor * Math.PI) * 40;
        y = floor * 10;
        ...
        if(t===2){
            config[3] = group3;
            bodys[i] = new OIMO.Body({type:'box', size:[w,h,d], pos:[x,y,z], rot:[0, 90+((N-pos)/N)*360, 0], move:true, world:world, config:config});
