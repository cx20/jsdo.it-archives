// forked from cx20's "LiquidFun を試してみるテスト（その９）" http://jsdo.it/cx20/y3GB
...
function TestLiquidTimer() {
    ...
    body = world.CreateBody(bd);
    shape = new b2EdgeShape;
    shape.Set(new b2Vec2(-1.5, 2.7), new b2Vec2(0.5, 2.1));
    body.CreateFixtureFromShape(shape, 0.1);

    makeFace(world, 1.0, 1.0);
    makeEye(world, 0.8, 0.9);
    makeEye(world, 1.2, 0.9);
    makeLip(world, 1.0, 1.5);
}

function makeFace(world, x, y) {
    var x0 = x;
    var y0 = y + 1;
    for (var t = 0.5; t < 20; t++) {
        var x1 = 0.9 * Math.sin(2 * Math.PI * t / 20) + x;
        var y1 = 0.6 * Math.cos(2 * Math.PI * t / 20) + y;
        bd = new b2BodyDef;
        body = world.CreateBody(bd);
        shape = new b2EdgeShape;
        shape.Set(new b2Vec2(x0, y0), new b2Vec2(x1, y1));
        body.CreateFixtureFromShape(shape, 0.1);
        x0 = x1;
        y0 = y1;
    }
    ...
    body = world.CreateBody(bd);
    shape = new b2EdgeShape;
    shape.Set(new b2Vec2(x0, y0), new b2Vec2(x0 + 0.1, y0 + 0.2));
    body.CreateFixtureFromShape(shape, 0.1);
}

function makeEye(world, x, y) {
    var x0 = x;
    var y0 = y + 0.15;
    for (var t = 0; t < 20; t++) {
        var x1 = 0.15 * Math.sin(2 * Math.PI * t / 20) + x;
        var y1 = 0.15 * Math.cos(2 * Math.PI * t / 20) + y;
        bd = new b2BodyDef;
        body = world.CreateBody(bd);
        shape = new b2EdgeShape;
        shape.Set(new b2Vec2(x0, y0), new b2Vec2(x1, y1));
...
function makeLip(world, x, y) {
    var x0 = 1 * Math.sin(2 * Math.PI * 8 / 20) + x;
    var y0 = 1 * Math.cos(2 * Math.PI * 8 / 20) + y;
    for (var t = 8; t <= 12; t++) {
        var x1 = 1 * Math.sin(2 * Math.PI * t / 20) + x;
        var y1 = 1 * Math.cos(2 * Math.PI * t / 20) + y;
        bd = new b2BodyDef;
        body = world.CreateBody(bd);
        shape = new b2EdgeShape;
        shape.Set(new b2Vec2(x0, y0), new b2Vec2(x1, y1));
