[WebGL] XenoGL を試してみるテスト（その３）

XenoGL は＠kfurumiya さんによる WebGL 2.0 用ライブラリです。
MatrixGL は ＠kfurumiya さんによる行列演算ライブラリです。

＜対応した点＞
・XenoGL で立方体を描くよう対応。
・クォータニオンで回転させるよう対応

＜参考＞
■ kotofurumiya/xenogl
https://github.com/kotofurumiya/xenogl

■ WebGL2をオブジェクト指向っぽく触れるライブラリXenoGLを公開しました
https://sbfl.net/blog/2018/02/12/webgl2-xenogl/

■ kotofurumiya/matrixgl
https://github.com/kotofurumiya/matrixgl

■ 各種 WebGL ライブラリで基本図形を表示してみる - Qiita
http://qiita.com/cx20/items/0fa19c96aa6470d98807
