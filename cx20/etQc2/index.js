...
function populate(n) {
    ...
    //add ground
    var ground = new OIMO.Body({size:[400, 40, 400], pos:[0,-20,0], rot:[0,0,5], world:world, config:config});
    addStaticBox([400, 40, 400], [0,-20,0], [0,0,5]);
    ...
    for (var i = 0; i < height; i++) {
        for (var j = i; j < height; j++) {
            for (var k = 0; k < depth; k++) {
                x = i * (h + 0.01) * 1.8 + h * 0.6 - 180;
                y = (k - (d - 1) * 0.5) + (k*d) + 10;
                z = (j - i * 0.5 - (height - 1) * 0.5) * 1.2 * (d * 1.05);
                config[3] = group3;
                bodys[index] = new OIMO.Body({type:'box', size:[w,h,d], pos:[x,y,z], rot:[0, 0, -90], move:true, world:world, config:config});
